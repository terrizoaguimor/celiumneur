#!/usr/bin/env bash
# yosys synthesis baseline for CeliumNeUR blocks on GF180 (pre-PnR evidence).
# Usage: synth_baseline.sh <name> <top_module> <verilog files...>
# Writes $SYNTH_OUT_DIR/<name>.log and <name>.json (stat -json),
# then prints: cell count / total area / ABC longest-path vs 20 ns target.
set -euo pipefail
if [ "$#" -lt 3 ]; then
  echo "usage: $0 <name> <top_module> <verilog files...>" >&2
  exit 2
fi
NAME=$1; TOP=$2; shift 2
LIB=/home/build/.volare/volare/gf180mcu/versions/c6d73a35f524070e85faff4a6a9eef49553ebc2b/gf180mcuA/libs.ref/gf180mcu_fd_sc_mcu7t5v0/lib/gf180mcu_fd_sc_mcu7t5v0__tt_025C_5v00.lib
export PATH=/opt/oss-cad-suite/bin:$PATH
OUT_DIR=${SYNTH_OUT_DIR:-/home/build/synth}
mkdir -p "$OUT_DIR"
OUT=$OUT_DIR/${NAME}.log
JSN=$OUT_DIR/${NAME}.json
YS=$(mktemp "/tmp/${NAME}_synth.XXXXXX.ys")
trap 'rm -f "$YS"' EXIT

if [ ! -f "$LIB" ]; then
  echo "missing liberty: $LIB" >&2
  exit 3
fi
for source in "$@"; do
  if [ ! -f "$source" ]; then
    echo "missing RTL source: $source" >&2
    exit 4
  fi
done

cat > "$YS" <<EOF
read_liberty -lib $LIB
read_verilog $@
hierarchy -check -top $TOP
proc; opt; fsm; opt; memory; opt
techmap; opt
dfflibmap -liberty $LIB
abc -liberty $LIB -D 20000
stat -liberty $LIB
tee -o $JSN stat -json -liberty $LIB
EOF
yosys -l "$OUT" -s "$YS" >/dev/null
python3 -m json.tool "$JSN" >/dev/null
echo "--- metrics for $NAME:"
grep -aE "Number of cells|Chip area|ABC: Delay" "$OUT" | tail -6 || true
sha256sum "$OUT" "$JSN" > "$OUT_DIR/${NAME}.sha256"
