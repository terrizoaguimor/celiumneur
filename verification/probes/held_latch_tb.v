...
module held_latch_tb;
    ...
    neuro_tile #(.GID_BASE(0)) dut (
        ...
        .cfg_soma_wdata(cfg_soma_wdata),
        .rb_dend_addr(5'd0), .rb_dend_rdata(),
        .rb_soma_addr(8'd0), .rb_soma_req(1'b0), .rb_soma_data(), .rb_soma_ready(),
        .axon_masks(16'h0004),
        .out_spk_valid(), .out_spk_pkt(), .out_spk_ready(1'b1),
        .out_stall_wit(), .tile_busy()
    );
    ...
    initial begin
        ...
        cfg_soma(8'd0, {16'd10, 1'b1, 4'd15, 8'd0, 8'd0, 3'b0, 16'd0});
        stim(8'd60);
        repeat (40) @(negedge clk);
        if (outs == 1 && !bad) begin
            $display("HELD-LATCH ok: pkt well-formed");
            $finish;
        end
